    ...
    <div className="min-h-[calc(100vh-5rem)] flex items-center justify-center px-4">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/[0.03] p-6 sm:p-7 shadow-[0_20px_60px_rgba(0,0,0,0.45)]">
        <div className="space-y-2">
          <div className="text-2xl font-black tracking-tight text-white">{mode === "login" ? "Login" : "Create Account"}</div>
          <div className="text-sm text-white/55">
            {mode === "login" ? "Sign in to continue." : "Create a new account to start."}
          </div>
        </div>
        ...
        <div className="mt-6 space-y-4">
          ...
          {mode === "register" && (
            <div>
              <div className="text-sm font-semibold text-white/80">Password Confirm</div>
              <input
          ...
          {mode === "login" ? (
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                disabled={submitting}
                className="flex-1 h-11 rounded-xl border border-white/10 bg-white/5 px-5 text-sm font-semibold text-white/80 hover:bg-white/10 transition disabled:opacity-40"
              >
                Cancel
              </button>
              <button
                ...
                onClick={handleSubmit}
                disabled={!canSubmit || submitting}
                className="flex-1 h-11 rounded-xl bg-blue-600 text-sm font-semibold text-white shadow-[0_15px_35px_rgba(59,130,246,0.45)] transition hover:bg-blue-500 disabled:opacity-40"
              >
